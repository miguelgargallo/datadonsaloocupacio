---
marp: true
theme: default
class: 
  - lead
  - invert
paginate: true
backgroundColor: #000000
color: #ffffff
style: |
  @import url('https://fonts.googleapis.com/css2?family=SF+Pro+Display:wght@300;400;500;600;700;900&display=swap');
  
  section {
    font-family: 'SF Pro Display', -apple-system, BlinkMacSystemFont, sans-serif;
    background: linear-gradient(135deg, #000000 0%, #1a1a1a 100%);
    color: #ffffff;
    justify-content: center;
    text-align: center;
  }
  
  h1 {
    font-size: 3.5rem;
    font-weight: 700;
    background: linear-gradient(45deg, #007AFF, #00D4FF);
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    margin-bottom: 0.5rem;
    letter-spacing: -0.02em;
  }
  
  h2 {
    font-size: 2.5rem;
    font-weight: 600;
    color: #ffffff;
    margin-bottom: 1rem;
    letter-spacing: -0.01em;
  }
  
  h3 {
    font-size: 1.5rem;
    font-weight: 500;
    color: #8E8E93;
    margin-bottom: 2rem;
    letter-spacing: 0.01em;
  }
  
  .hero {
    background: radial-gradient(circle at center, #007AFF20 0%, #000000 70%);
    padding: 4rem 2rem;
  }
  
  .feature-grid {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 2rem;
    margin: 2rem 0;
  }
  
  .feature-card {
    background: rgba(255,255,255,0.05);
    border-radius: 16px;
    padding: 2rem;
    border: 1px solid rgba(255,255,255,0.1);
    backdrop-filter: blur(20px);
  }
  
  .metric {
    font-size: 4rem;
    font-weight: 900;
    background: linear-gradient(45deg, #00D4FF, #007AFF);
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
  }
  
  .company-logo {
    font-size: 2rem;
    font-weight: 700;
    color: #007AFF;
    margin-bottom: 0.5rem;
  }
  
  .tagline {
    font-size: 1.2rem;
    color: #8E8E93;
    font-weight: 300;
  }
  
  .cta-button {
    background: linear-gradient(45deg, #007AFF, #00D4FF);
    color: white;
    padding: 1rem 3rem;
    border-radius: 50px;
    font-weight: 600;
    font-size: 1.1rem;
    border: none;
    margin-top: 2rem;
    display: inline-block;
    text-decoration: none;
  }

---

<!-- _class: hero -->

# Water Intelligence
### Redefining Urban Consumption Analytics

**Transforming Barcelona's Water Data into Strategic Insights**

<div class="company-logo">CONAN DATA</div>
<div class="tagline">Data Consultancy • Barcelona • 2025</div>

---

## The Challenge

<div class="feature-grid">
<div class="feature-card">

### 📊 **Complex Data**
Multiple districts, usage types, and consumption patterns create analytical complexity

</div>
<div class="feature-card">

### ⏰ **Real-time Needs**
Decision makers require instant access to consumption insights

</div>
<div class="feature-card">

### 🎯 **Strategic Impact**
Water management directly affects urban sustainability and economic efficiency

</div>
<div class="feature-card">

### 💡 **Actionable Intelligence**
Raw data needs transformation into clear, strategic recommendations

</div>
</div>

---

## Our Solution

### **Water Consumption Analytics Dashboard**

**Real-time intelligence platform designed for Barcelona's water ecosystem**

- Interactive visualization of consumption patterns
- District-by-district performance analysis  
- Predictive insights and trend identification
- Executive-ready reporting capabilities

---

## Key Features

<div class="feature-grid">
<div class="feature-card">

### 🚀 **Instant Analytics**
Real-time dashboard with interactive filtering and visualization

</div>
<div class="feature-card">

### 📈 **Predictive Intelligence**
Advanced time-series analysis reveals consumption trends and patterns

</div>
<div class="feature-card">

### 🎨 **Executive Design**
Apple-inspired interface ensures professional presentation to stakeholders

</div>
<div class="feature-card">

### ⚡ **Performance Optimized**
Tesla-level efficiency in data processing and visualization rendering

</div>
</div>

---

## The Numbers

<div style="display: flex; justify-content: space-around; align-items: center; margin: 3rem 0;">

<div style="text-align: center;">
<div class="metric">365</div>
<div style="color: #8E8E93;">Days of Data</div>
</div>

<div style="text-align: center;">
<div class="metric">10+</div>
<div style="color: #8E8E93;">Districts Analyzed</div>
</div>

<div style="text-align: center;">
<div class="metric">3</div>
<div style="color: #8E8E93;">Usage Categories</div>
</div>

</div>

### Comprehensive coverage of Barcelona's water consumption ecosystem

---

## Live Dashboard Demo

### **Interactive Experience**

- **Time-series Analysis**: Consumption patterns over time
- **District Comparison**: Performance across Barcelona neighborhoods  
- **Usage Breakdown**: Commercial, Domestic, and Industrial insights
- **Heat Maps**: Visual consumption intensity mapping

**Built with Streamlit + Plotly for maximum interactivity**

---

## Technology Stack

<div class="feature-grid">
<div class="feature-card">

### 🖥️ **Frontend**
Streamlit with custom Apple-inspired CSS styling

</div>
<div class="feature-card">

### 📊 **Analytics**
Pandas, NumPy for high-performance data processing

</div>
<div class="feature-card">

### 📈 **Visualization**
Plotly for interactive, publication-ready charts

</div>
<div class="feature-card">

### ⚙️ **Architecture**
Modular, scalable design for enterprise deployment

</div>
</div>

---

## Business Impact

### **Strategic Value Creation**

<div style="text-align: left; max-width: 800px; margin: 0 auto;">

**For Water Utilities:**
- Optimize resource allocation across districts
- Identify peak usage patterns for capacity planning
- Monitor efficiency metrics in real-time

**For City Planners:**
- Support sustainable urban development initiatives
- Analyze consumption trends for infrastructure investment
- Generate data-driven policy recommendations

**For Executive Teams:**
- Access professional reports for board presentations
- Track KPIs with visual performance dashboards
- Make informed strategic decisions with confidence

</div>

---

## Why Conan Data?

<div class="feature-grid">
<div class="feature-card">

### 🎯 **Catalan Expertise**
Deep understanding of local market needs and regulatory requirements

</div>
<div class="feature-card">

### 🚀 **Startup Agility**
Rapid development cycles and innovative solution approaches

</div>
<div class="feature-card">

### 🏆 **Enterprise Quality**
Professional-grade deliverables with startup flexibility

</div>
<div class="feature-card">

### 📈 **Growth Partnership**
Long-term consulting relationship focused on your success

</div>
</div>

---

## Implementation Roadmap

### **Phase 1: Foundation** *(Week 1-2)*
- Data integration and validation
- Core dashboard development
- Basic visualization implementation

### **Phase 2: Enhancement** *(Week 3-4)*
- Advanced analytics features
- Custom styling and branding
- Performance optimization

### **Phase 3: Excellence** *(Week 5-6)*
- Predictive modeling integration
- Executive reporting capabilities
- Training and documentation

---

## What's Next?

### **Future Roadmap**

- **AI Integration**: Machine learning for consumption prediction
- **Mobile Applications**: Native iOS and Android apps
- **API Development**: RESTful services for third-party integration
- **Real-time Streaming**: Live data connection capabilities

### **Scalability Ready**
Built to grow with your organization's evolving needs

---

## Let's Transform Your Data

<div style="margin: 3rem 0;">

### **Ready to unlock your water consumption intelligence?**

**Schedule a consultation with Conan Data**

<div class="cta-button">Start Your Data Journey</div>

</div>

### Contact Information
**Email**: hello@conandata.com  
**Location**: Barcelona, Catalunya  
**Expertise**: Data Strategy • Analytics • Visualization

---

<!-- _class: hero -->

# Thank You

### **Questions & Discussion**

<div class="company-logo">CONAN DATA</div>
<div class="tagline">Transforming Data into Strategic Advantage</div>

**Barcelona • 2025**
